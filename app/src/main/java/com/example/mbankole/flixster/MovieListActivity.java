...
public class MovieListActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // init the adapter - movies array can not be reinitialized after this
        adapter = new MovieAdapter(movies);

        //resolve the reference for the recyclerview, choose a layout manager, and the adapter
            ...
            public void onSuccess(int statusCode, Header[] headers, JSONObject response) {
                ...
                try {
                    ...
                    for (int i = 0; i < results.length(); i++) {
                        ...
                        movies.add(movie);
                        //notify adapter that items are added
                        adapter.notifyItemInserted(movies.size() - 1);
                    }
                    Log.i(TAG, String.format("Loaded %s Movies", results.length()));
                } catch (JSONException e) {
                    logError("Failed to parse now playing movie json response", e, true);
                }
            }
